...
class PreApproveEntryController extends Controller
{
    ...
    {
        $gatekeeper = GateKeeper::where('subadminid',  $subadminid)->join('users', 'users.id', '=', 'gatekeepers.gatekeeperid')->get();
    ...
    {
        $unapproveentries = Preapproveentry::where('gatekeeperid', '=', $userid)->join('users', 'users.id', '=', 'preapproveentries.userid')->where('status', 0)->select(
